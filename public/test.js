...
var LDAvis = function (to_select, data_or_file_name, color1, color2) {
  ...
  function visualize(data) {
    ...
    svg
      .append('text')
      .text('Мапа відстаней між темами')
    ...
    var chart = svg
      ...
      .attr(
        'transform',
        'translate(' + +(mdswidth + margin.left + termwidth) + ',' + 2 * margin.top + ')'
      )
      .attr('id', barFreqsID);
    ...
    d3.select('#' + barFreqsID)
      ...
      .style('dominant-baseline', 'middle')
      .text('Частота слова для певної теми');
    ...
    var title = chart
      ...
      .style('text-anchor', 'middle')
      .style('font-size', '16px')
      .text('Топ-' + R + ' Найпопулярніших слів');
    ...
    var xAxis = d3.axisTop(x).tickSize(0).ticks(6);

    chart.append('g').attr('x', 'xaxis').call(xAxis);
    ...
    function init_forms(topicID, lambdaID, visID) {
      ...
      topicDiv.setAttribute(
        'style',
        'padding: 5px; background-color: rgba(0, 0, 0, 0.02); border-radius: 8px; display: flex; align-items: center; justify-content: center; gap: 5px; width: ' +
          mdswidth +
          'px;'
      );
      ...
      topicLabel.innerHTML = "Тема: <span id='" + topicID + "-value'></span>";
      ...
      previous.setAttribute(
        'style',
        'background-color: #1677ff; padding: 4px 15px; color: #fff; border: none; border-radius: 8px; line-height: 1.57'
      );
      previous.innerHTML = 'Попередня тема';
    ...
    function reorder_bars(increase) {
      ...
      if (increase) {
        ...
        newaxis.transition().duration(duration).call(xAxis).transition().duration(duration);
      } else {
        graybarsEnter
        ...
        // https://github.com/mbostock/d3/wiki/Transitions#wiki-d3_ease
        newaxis.transition().duration(duration).transition().duration(duration).call(xAxis);
      }
    }
    ...
    function topic_on(circle) {
      ...
      d3.select('#' + barFreqsID)
        ...
        .style('text-anchor', 'middle')
        .style('font-size', '16px')
        .text('Top-' + R + ' Найпопулярніших слів для Теми ' + topics + ' (' + Freq + '% слів)');
      ...
      for (var i = 0; i < dat2.length; i++) {
        dat2[i].relevance =
          lambda.current * dat2[i].logprob + (1 - lambda.current) * dat2[i].loglift;
      }
      ...
      // redraw x-axis
      d3.selectAll(to_select + ' .xaxis').call(xAxis);
    }
    ...
    function topic_off(circle) {
      ...
      var title = d3
        .selectAll(to_select + ' .bubble-tool')
        .text('Топ-' + R + ' Найпопулярніших слів');
      // title
      //   .append('tspan')
      //   .attr('baseline-shift', 'super')
    ...
    function term_on(term) {
      ...
      var rScaleCond = d3.scaleSqrt().domain([0, 1]).range([0, rMax]);
      ...
      d3.select(to_select + ' .circleGuideTitle').text(
        "Розподіл тем зі словом = '" + term.innerHTML + "'"
      );
    }
    ...
    function term_off(term) {
      ...
      d3.select(to_select + ' .circleGuideTitle').text('Загальний розподіл тем');
      // d3.select(to_select + ' .circleGuideLabelLarge').text(defaultLabelLarge);
